...
const JournalView = {
    ...
    bindEvents() {
        ...
        // Save edited trade
        this.elements.editTradeForm.addEventListener('submit', (e) => {
            e.preventDefault();
            this.saveEditedTrade();
        });

        // Direction toggles (New)
        ...
        // Filters
        this.elements.filterAccount.addEventListener('change', () => {
            this.currentFilters.account = this.elements.filterAccount.value;
            this.renderAll();
        });
        this.elements.filterResult.addEventListener('change', () => {
            this.currentFilters.result = this.elements.filterResult.value;
            this.renderAll();
        });
        this.elements.filterPair.addEventListener('change', () => {
    ...
    openEditFromDetail() {
        if (!this.viewingTradeId) return;
        this.closeModal(this.elements.detailModal);
        this.openEditModal(this.viewingTradeId);
    },
    ...
    deleteFromDetail() {
        if (this.viewingTradeId) {
            this.deleteTrade(this.viewingTradeId);
        }
    },
    ...
    openDetailModal(tradeId) {
        ...
        this.elements.detailContent.innerHTML = `
            <div class="detail-row">
                ...
                <span class="detail-value">${UI.escapeHTML(accountName)}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Date</span>
                <span class="detail-value">${FORMATTERS.fullDate.format(new Date(trade.date + 'T00:00:00'))}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Pair</span>
                <span class="detail-value">${UI.escapeHTML(trade.pair)}</span>
            </div>
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value ${resultClass}">${trade.result}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">P&L</span>
                <span class="detail-value ${resultClass}">${pnlSign}${pnlFormatted}</span>
            </div>
            <div class="detail-row">
                <span class="detail-label">Risk</span>
                <span class="detail-value">${trade.riskPercent || '--'}%</span>
            </div>
            ${trade.entry ? `
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">${trade.sl}</span>
            </div>` : ''}
            ${trade.tp ? `
            <div class="detail-row">
                <span class="detail-label">Take Profit</span>
                <span class="detail-value">${trade.tp}</span>
            </div>` : ''}
            ${trade.lotSize ? `
            <div class="detail-row">
            ...
            <div class="detail-row">
                ...
                <span class="detail-value">${'★'.repeat(trade.rating || 0)}${'☆'.repeat(5 - (trade.rating || 0))}</span>
            </div>
            ${trade.wentWell ? `
            <div class="detail-section-title">What went well</div>
            <div class="detail-text">${UI.escapeHTML(trade.wentWell)}</div>` : ''}
            ${trade.mistakes ? `
            <div class="detail-section-title">Mistakes made</div>
            <div class="detail-text">${UI.escapeHTML(trade.mistakes)}</div>` : ''}
            ${trade.lesson ? `
            <div class="detail-section-title">Lesson learned</div>
            <div class="detail-text">${UI.escapeHTML(trade.lesson)}</div>` : ''}
        `;
    ...
    collectFormData(mode) {
        ...
        const date = mode === 'new'
            ? this.elements.tradeDate.value
            : this.elements.editTradeDate.value;

        const pair = mode === 'new'
            ? this.elements.tradePair.value
            : this.elements.editTradePair.value;

        const result = mode === 'new'
            ? this.elements.tradeResult.value
            : this.elements.editTradeResult.value;
        ...
        return {
            ...
            result,
            pnl,
            riskPercent: mode === 'new'
                ? (parseFloat(this.elements.tradeRiskPercent.value) || 0)
                : (parseFloat(this.elements.editTradeRiskPercent.value) || 0),
            entry: mode === 'new'
                ? (this.elements.tradeEntry.value ? parseFloat(this.elements.tradeEntry.value) : null)
                : (this.elements.editTradeEntry.value ? parseFloat(this.elements.editTradeEntry.value) : null),
            sl: mode === 'new'
                ? (this.elements.tradeSL.value ? parseFloat(this.elements.tradeSL.value) : null)
                : (this.elements.editTradeSL.value ? parseFloat(this.elements.editTradeSL.value) : null),
            tp: mode === 'new'
                ? (this.elements.tradeTP.value ? parseFloat(this.elements.tradeTP.value) : null)
                : (this.elements.editTradeTP.value ? parseFloat(this.elements.editTradeTP.value) : null),
            lotSize: mode === 'new'
                ? (this.elements.tradeLotSize.value ? parseFloat(this.elements.tradeLotSize.value) : null)
                : (this.elements.editTradeLotSize.value ? parseFloat(this.elements.editTradeLotSize.value) : null),
            rating: mode === 'new'
                ? parseInt(this.elements.tradeRating.value) || 0
                : parseInt(this.elements.editTradeRating.value) || 0,
    ...
    getFilteredTrades() {
        ...
        if (this.currentFilters.account !== 'all') {
            trades = trades.filter(t => t.accountId === this.currentFilters.account);
        }
        if (this.currentFilters.result !== 'all') {
            trades = trades.filter(t => t.result === this.currentFilters.result);
        }
        if (this.currentFilters.pair !== 'all') {
                ...
                <div class="trade-entry-body">
                    <div class="trade-entry-details-grid">
                        <div class="trade-entry-detail">
                            <span class="trade-entry-detail-label">Account</span>
                            <span class="trade-entry-detail-value">${UI.escapeHTML(accountName)}</span>
                        </div>
                        ${trade.entry ? `
                        <div class="trade-entry-detail">
                            <span class="trade-entry-detail-label">Entry</span>
                            <span class="trade-entry-detail-value">${trade.entry}</span>
                        </div>` : '<div></div>'}
                        ${trade.sl ? `
                        <div class="trade-entry-detail">
                            <span class="trade-entry-detail-label">Stop Loss</span>
                            <span class="trade-entry-detail-value">${trade.sl}</span>
                        </div>` : '<div></div>'}
                        ${trade.tp ? `
                        <div class="trade-entry-detail">
                            <span class="trade-entry-detail-label">Take Profit</span>
                            <span class="trade-entry-detail-value">${trade.tp}</span>
                        </div>` : '<div></div>'}
                        ${trade.lotSize ? `
                        <div class="trade-entry-detail">
                            <span class="trade-entry-detail-label">Lot Size</span>
                            <span class="trade-entry-detail-value">${trade.lotSize}</span>
                        </div>` : '<div></div>'}
                        ${trade.riskPercent ? `
                        <div class="trade-entry-detail">
                    ...
                    <div class="trade-entry-reflection">
                        <div class="trade-entry-reflection-block">
                            <div class="trade-entry-reflection-label">What went well</div>
                            <div class="trade-entry-reflection-text">${UI.escapeHTML(trade.wentWell || '--')}</div>
                        </div>
                        <div class="trade-entry-reflection-block">
                            <div class="trade-entry-reflection-label">Mistakes made</div>
                            <div class="trade-entry-reflection-text">${UI.escapeHTML(trade.mistakes || '--')}</div>
                        </div>
                        <div class="trade-entry-reflection-block">
                            <div class="trade-entry-reflection-label">Lesson learned</div>
                            <div class="trade-entry-reflection-text">${UI.escapeHTML(trade.lesson || '--')}</div>
                        </div>
                    </div>
                    ` : ''}
